...
class Journal_EntriesDB:
    def __init__(self):
        self._connection = sqlite3.connect("journal_entries.db") #Use the journal_entries.db file name for the journal_entries sqlite database.
    ...
    def get_journal_entry(self, entry_id):
        ...
        return journal_entry #will return None if the record does not exist
    ...
    def update_journal_entry(self, date, entry, author, mood, mood_scale, entry_id):
        ...
        self._connection.commit()
    def get_users(self): #comment out this function
        self._cursor.execute("SELECT * FROM users")
        users = self._cursor.fetchall()
        return users
    def get_user_with_id(self, user_id): #Comment out this function
        data = [user_id]
        self._cursor.execute("SELECT * FROM users WHERE id = ?", data)
        user = self._cursor.fetchone()
        return user #will return None if the record does not exist
    ...
    def check_user(self, email):
        ...
        return user_check
    def delete_user(self, user_id): #Comment out this function
        data = [user_id]
        self._cursor.execute("DELETE FROM users WHERE id = ?", data)
        self._connection.commit()
